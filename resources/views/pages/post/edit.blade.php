...
@section('content')
    ...
    <div class="conteiner">
        ...
        <form action="#" class="form-add" method="post">
            ...
            <input type="text" class="form-add__titel style__all" id="titel" placeholder="headline" name="title"
            ...
            <select name="category_id" id="category" class="form-add__category  style__all">
                ...
                @foreach($categories as $categoryforID)
                    <option class="form-add__category__inner" @if ($categoryforID->id==old('category_id')) selected
                            @endif value="{{$categoryforID->id}}">{{$categoryforID->title}}</option>
            ...
            <textarea class="style__all" name="body" id="post" cols="30" rows="10"
                      placeholder="Write text">{{old('body')?? $post->body}}</textarea>
            ...
            <div class="checkbox">
                @foreach($tags as $tag)
                    <div class="checkbox-inner">
                        @if(old('tags_id'))
                            <input type="checkbox" class="checkbox__input check"
                                   @if(array_search($tag->id,old('tags_id'))!==false) checked @endif name="tags_id[]"
                                   value={{$tag->id}}>
                        @else
                            <input type="checkbox" class="checkbox__input check"
                                   @if(array_search($tag->id,$post->tags->pluck('id')->toArray())!==false) checked
                                   @endif name="tags_id[]" value={{$tag->id}}>
                        @endif
